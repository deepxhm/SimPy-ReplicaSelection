...
class Client():
    ...
    def sort(self, originalReplicaSet):
        ...
        if(self.REPLICA_SELECTION_STRATEGY == "RAND"):
            ...
            t = sum(map(lambda x: self.responseTimesMap[x], replicaSet))
            total = sum(map(lambda x: (t-self.responseTimesMap[x]), replicaSet))
            ...
            if (total != 0):
                for entry in replicaSet:
                    cumSum += (t-self.responseTimesMap[entry])
                    if (selection < cumSum):
                        nodeToSelect = entry
                        break
                    i += 1
        ...
        elif(self.REPLICA_SELECTION_STRATEGY == "WRRL"):
            # Weighted random proportional to OSK
            #m = {}
            #for each in replicaSet:
            #    if ("serviceTime" not in self.expectedDelayMap[each]):
            #        m[each] = 0.0
            ...
            #        m[each] = self.expectedDelayMap[each]["serviceTime"]
            #replicaSet.sort(key=m.get)
            t = sum(map(lambda x: self.pendingRequestsMap[x], replicaSet))
            total = sum(map(lambda x: (t-self.pendingRequestsMap[x]), replicaSet))
            selection = random.uniform(0, total)
            cumSum = 0
            nodeToSelect = None
            i = 0
            if (total != 0):
                for entry in replicaSet:
                    cumSum += (t-self.pendingRequestsMap[entry])
                    if (selection < cumSum):
                        nodeToSelect = entry
                        break
                    i += 1
        ...
        elif(self.REPLICA_SELECTION_STRATEGY == "refresh"):
            # refresh selection
            sortMap = {}
            for replica in originalReplicaSet:
                sortMap[replica] = self.computeExpectedDelayRefresh(replica)
            replicaSet.sort(key=sortMap.get)
        ...
        elif(self.REPLICA_SELECTION_STRATEGY == "2CR"):
            # choose by OSK
            sortMap = {}
            random.shuffle(replicaSet)
            twoChoices, rest = replicaSet[:2], replicaSet[2:]
            for replica in twoChoices:
                sortMap[replica] = self.computeMetricTwoChoicesLOR(replica)
            for replica in rest:
                sortMap[replica] = float("inf")
            replicaSet.sort(key=sortMap.get)

        elif(self.REPLICA_SELECTION_STRATEGY == "InterC"):
            # Uses the Queue-size-estimate ^ b and service time product, but
            # unlike the C3 mechanism, desynchronizes using a two
            # choices selection instead of the outstanding requests.
            # InterC is different with 2C on shuffule
            sortMap = {}
            scorelist = []
	    for replica in replicaSet:
	    	sortMap[replica] = self.computeMetricInterC(replica)
	    	scorelist.append(self.computeMetricInterC(replica))
	    replicaSet.sort(key=sortMap.get)
            
            # record average for mongoDB
 
            average = sum(scorelist)/len(scorelist) + 15

            # computeing the threshold
            threshold = 0
            for key in sortMap:
	    	if sortMap[key] < average:
			threshold = threshold + 1
	    # twoChoices, rest = replicaSet[:2], replicaSet[2:]
	    
	    # replicaSet[:2] = random.shuffle(twoChoices)
            
	    InterChoices = replicaSet[:threshold]
	    random.shuffle(InterChoices)
	    replicaSet[:threshold] = InterChoices
	    
	    """
            random.shuffle(replicaSet)
            twoChoices, rest = replicaSet[:2], replicaSet[2:]
            for replica in twoChoices:
                sortMap[replica] = self.computeMetricNewTwoChoices(replica)
            for replica in rest:
                sortMap[replica] = float("inf")
            replicaSet.sort(key=sortMap.get)
	    """
        elif(self.REPLICA_SELECTION_STRATEGY == "TLL"):
            #twochoices+LOR+LRPT
            replicaSet.sort(key=self.pendingRequestsMap.get)
	    scorelist = []
	    for replica in replicaSet:
		#print self.pendingRequestsMap[replica]
		scorelist.append(self.pendingRequestsMap[replica])
		...
		#print i, scorelist[i]
            ...
            #print threshold, len(replicaSet), "\n" 
	    InterChoices = replicaSet[:threshold]
	    InterChoices.sort(key=self.responseTimesMap.get)
	    #print len(InterChoices)
	    #LRPT+refresh
	    #sortMap = {}
            #for replica in InterChoices:
            #    sortMap[replica] = self.computeExpectedDelayRefresh(replica)
	    #InterChoices.sort(key=sortMap.get)
	    replicaSet[:threshold] = InterChoices
    ...
    def computeExpectedDelayRefresh(self, replica):
	    ...
	    if (Simulation.now() - metricMap["receiveTime"] < 100):
            ...
            total += twiceNetworkLatency
            self.edScoreMonitor.observe("%s %s %s %s" %
                                        (replica.id,
                                         metricMap["queueSizeAfter"],
                                         metricMap["serviceTime"],
                                         total))
    ...
    def computeMetricTwoChoices(self, replica):
        ...
        if (len(self.expectedDelayMap[replica]) != 0):
            ...
            twiceNetworkLatency = metricMap["nw"]
            #total += twiceNetworkLatency
            total = self.responseTimesMap[replica]
            self.edScoreMonitor.observe("%s %s %s %s" %
                                        (replica.id,
                                         metricMap["queueSizeAfter"],
                                         metricMap["serviceTime"],
    ...
    def computeMetricTwoChoicesLOR(self, replica):
        total = self.pendingRequestsMap[replica]
        return total

    # scoring for NewTwoChoices
    def computeMetricNewTwoChoices(self, replica):
        total = 0
        if (len(self.expectedDelayMap[replica]) != 0):
            metricMap = self.expectedDelayMap[replica]
            twiceNetworkLatency = metricMap["nw"]
    ...
    def computeMetricInterC(self, replica):
        total = 0
        if (len(self.expectedDelayMap[replica]) != 0):
            metricMap = self.expectedDelayMap[replica]
            twiceNetworkLatency = metricMap["nw"]
            # the default RTT is 0.5
            total = 0.5
            self.edScoreMonitor.observe("%s %s %s %s" %
                                        (replica.id,
                                         metricMap["queueSizeAfter"],
...
class ResponseHandler(Simulation.Process):
    ...
    def run(self, client, task, replicaThatServed):
        ...
        metricMap["receiveTime"] = Simulation.now() 
